	...
	<view :style='{"width":"100%","padding":"40rpx 0 0","position":"relative","background":"#fff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"0 0 48rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"4rpx 40rpx","margin":"0 0 8rpx 0","borderColor":"#ccc","alignItems":"center","borderRadius":"0","borderWidth":"0 0 0px 0","background":"#fff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"2rpx solid #666","padding":"0px 24rpx","margin":"0px","color":"rgb(0, 0, 0)","borderRadius":"8rpx","flex":"1","background":"rgba(255, 255, 255, 0)","fontSize":"28rpx","height":"80rpx"}' :disabled="ro.louyumingcheng" v-model="ruleForm.louyumingcheng" placeholder="楼宇名称"  type="text"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`louyumingcheng`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				//跨表计算判断
				var obj;
				if((!this.ruleForm.louyumingcheng)){
					this.$utils.msg(`楼宇名称不能为空`);
					return
				}
				//更新跨表属性
				var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`louyumingcheng`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`louyumingcheng`, this.ruleForm);
						}else{
							oet = await this.$api.add(`louyumingcheng`, this.ruleForm);
						}
						if(this.cross){
