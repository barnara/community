	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="标题名称" prop="biaotimingcheng" >
				...
				<el-form-item class="input" v-else-if="ruleForm.tousuriqi" label="投诉日期" prop="tousuriqi" >
					<el-input v-model="ruleForm.tousuriqi" placeholder="投诉日期" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="投诉状态" prop="tousuzhuangtai" >
					<el-select :disabled="ro.tousuzhuangtai" v-model="ruleForm.tousuzhuangtai" placeholder="请选择投诉状态" >
						<el-option
							v-for="(item,index) in tousuzhuangtaiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="投诉状态" prop="tousuzhuangtai" >
					<el-input v-model="ruleForm.tousuzhuangtai"
			...
			<el-form-item v-else-if="ruleForm.tousuneirong" label="投诉内容" prop="tousuneirong" >
				<span class="text">{{ruleForm.tousuneirong}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `tousufuwu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tousufuwuCrossAddOrUpdateFlag = false;
											this.parent.search();
