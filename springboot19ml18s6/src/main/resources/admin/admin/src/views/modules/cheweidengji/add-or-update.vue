	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item v-else class="input" label="车位编号" prop="cheweibianhao" >
				...
				<el-form-item class="upload" v-else-if="ruleForm.cheweitupian" label="车位图片" prop="cheweitupian" >
					<img v-if="ruleForm.cheweitupian.substring(0,4)=='http'&&ruleForm.cheweitupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.cheweitupian" width="100" height="100">
					<img v-else-if="ruleForm.cheweitupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.cheweitupian.split(',')[0]" width="100" height="100">
				...
				<el-form-item v-else class="input" label="车位位置" prop="cheweiweizhi" >
				...
				<el-form-item v-else class="input" label="车位状态" prop="cheweizhuangtai" >
					<el-input v-model="ruleForm.cheweizhuangtai"
						placeholder="车位状态" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="车位类型" prop="cheweileixing" >
				...
				<el-form-item v-else class="input" label="车位类型" prop="cheweileixing" >
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				this.cheweileixingOptions = "标准,子母,大型".split(',')
				this.$http({
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `cheweidengji/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.cheweidengjiCrossAddOrUpdateFlag = false;
											this.parent.search();
