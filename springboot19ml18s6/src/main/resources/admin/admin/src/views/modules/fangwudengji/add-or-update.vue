<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="select" v-if="type!='info'"  label="楼宇名称" prop="louyumingcheng" >
					<el-select :disabled="ro.louyumingcheng" v-model="ruleForm.louyumingcheng" placeholder="请选择楼宇名称" >
						<el-option
							v-for="(item,index) in louyumingchengOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="楼宇名称" prop="louyumingcheng" >
					<el-input v-model="ruleForm.louyumingcheng"
						placeholder="楼宇名称" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="单元号" prop="danyuanhao" >
					<el-select :disabled="ro.danyuanhao" v-model="ruleForm.danyuanhao" placeholder="请选择单元号" >
						<el-option
							v-for="(item,index) in danyuanhaoOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="房间号" prop="fangjianhao" >
					<el-input v-model="ruleForm.fangjianhao" placeholder="房间号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="住宅面积" prop="zhuzhaimianji" >
					<el-input v-model="ruleForm.zhuzhaimianji" placeholder="住宅面积" clearable  :readonly="ro.zhuzhaimianji"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="住宅面积" prop="zhuzhaimianji" >
					<el-input v-model="ruleForm.zhuzhaimianji" placeholder="住宅面积" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="产权" prop="chanquan" >
					<el-select :disabled="ro.chanquan" v-model="ruleForm.chanquan" placeholder="请选择产权" >
						<el-option
							v-for="(item,index) in chanquanOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="产权" prop="chanquan" >
					<el-input v-model="ruleForm.chanquan"
						placeholder="产权" readonly></el-input>
				</el-form-item>
				...
				<el-form-item v-else class="input" label="房屋类型" prop="fangwuleixing" >
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='zhuzhaimianji'){
							this.ruleForm.zhuzhaimianji = obj[o];
							this.ro.zhuzhaimianji = true;
							continue;
						}
						if(o=='chanquan'){
							this.ruleForm.chanquan = obj[o];
							this.ro.chanquan = true;
							continue;
						}
						if(o=='fangwuleixing'){
				...
				this.$http({
					...
					if (data && data.code === 0) {
						var json = data.data;
						if(((json.louyumingcheng!=''&&json.louyumingcheng) || json.louyumingcheng==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.louyumingcheng = json.louyumingcheng
							this.ro.louyumingcheng = true;
						}
						if(((json.danyuanhao!=''&&json.danyuanhao) || json.danyuanhao==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.danyuanhao = json.danyuanhao
							this.ro.danyuanhao = true;
						}
					} else {
				...
				this.$http({
					...
					if (data && data.code === 0) {
						this.louyumingchengOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
				this.$http({
					...
					if (data && data.code === 0) {
						this.danyuanhaoOptions = data.data;
					} else {
						this.$message.error(data.msg);
					}
				});
				this.chanquanOptions = "40年产权,70年产权".split(',')
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `fangwudengji/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fangwudengjiCrossAddOrUpdateFlag = false;
											this.parent.search();
