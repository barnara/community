	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusszhoubianfuwu`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusszhoubianfuwu`, params);
				...
				//跨表计算
						let oet = {}
						if(this.ruleForm.id){
							await this.$api.update(`discusszhoubianfuwu`, this.ruleForm);
						}else{
							oet = await this.$api.add(`discusszhoubianfuwu`, this.ruleForm);
						}
						uni.setStorageSync('discusszhoubianfuwuCleanType',true);
						if(this.cross){
