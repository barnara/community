		...
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
			<view :style='{"alignContent":"flex-start","padding":"24rpx","margin":"0 auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"40rpx auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"20"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#666","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="userid&&isAuth('discusszhoubianfuwu','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#666","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="!userid&&isAuthFront('discusszhoubianfuwu','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusszhoubianfuwuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusszhoubianfuwuCleanType')
				this.mescroll.num = 1
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discusszhoubianfuwu', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','discusszhoubianfuwu');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','discusszhoubianfuwu');
			...
			async init(type=1){
				...
				let res = await this.$api.info('discusszhoubianfuwu', this.id);
				let reg=new RegExp('http://localhost:8080/springboot19ml18s6/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('discusszhoubianfuwu',this.detail)
				}
			},
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussdiscusszhoubianfuwu', {
						page: mescroll.num,
						limit: 10,
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscusszhoubianfuwu',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			async onCommentTap() {
				...
				let res = {}
				this.$utils.jump(`../discussdiscusszhoubianfuwu/add-or-update?refid=${this.id}` )
			},
			delClick(id){
				let that = this
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusszhoubianfuwu',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusszhoubianfuwu',that.detail)
							setTimeout(()=>{
