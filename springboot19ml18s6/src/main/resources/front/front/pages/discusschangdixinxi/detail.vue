		...
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#fff","height":"auto"}'>
			<view :style='{"alignContent":"flex-start","padding":"24rpx","margin":"0 auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","height":"auto","zIndex":"9"}' class="detail-content">
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"0","margin":"40rpx auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"20"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#666","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="userid&&isAuth('discusschangdixinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","boxShadow":"none","margin":"0 10rpx 10rpx 0","color":"rgb(255, 255, 255)","borderRadius":"0","background":"#666","width":"auto","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx","order":"8"}' v-if="!userid&&isAuthFront('discusschangdixinxi','私聊')" @tap="chatClick">联系TA</button>
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('discusschangdixinxi', this.id);
		...
		methods: {
			...
			async comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					await this.$api.update('discussdiscusschangdixinxi',row)
					this.$utils.msg('点赞成功');
				}else {
					row.thumbsupnum--
					let arr = String(row.tuserids).split(',')
			...
			delClick(id){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussdiscusschangdixinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('discusschangdixinxi',that.detail)
							setTimeout(()=>{
